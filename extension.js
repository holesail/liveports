...
function activate (context) {
  ...
  const share = vscode.commands.registerCommand('holesail-liveports.liveports-share', async function () {
    ...
    const isPrivate = privateChoice === 'Yes'
    try {
      ...
      console.log(`${address}:${port} is now live on the following key => ${url}`)

      vscode.window.showInformationMessage(`${address}:${port} is now live on the following key: ${url}`, 'Copy key').then(selected => {
      ...
      activeServers.set(id, { holesail, port, address, protocol: proto, secure: isPrivate, url })
    } catch (e) {
      vscode.window.showErrorMessage(`Failed to start server: ${e.message}`)
    }
  })

  const connect = vscode.commands.registerCommand('holesail-liveports.liveports-connect', async function () {
    ...
    const secure = data.secure !== undefined ? data.secure : true
    ...
    const protocol = result.protocol ? result.protocol.toUpperCase() : (udp ? 'UDP' : 'TCP')
  ...
  const lookup = vscode.commands.registerCommand('holesail-liveports.liveports-lookup', async function () {
    const url = await vscode.window.showInputBox({
    ...
    const data = Holesail.urlParser(url)
    const secure = data.secure !== undefined ? data.secure : false
  ...
  const destroyServer = vscode.commands.registerCommand('holesail-liveports.destroyServer', async function (id) {
  ...
  const disconnectClient = vscode.commands.registerCommand('holesail-liveports.disconnectClient', async function (id) {
  ...
  const restartServer = vscode.commands.registerCommand('holesail-liveports.restartServer', async function (id) {
    const entry = activeServers.get(id)
    if (entry) {
      try {
        await entry.holesail.close()
  ...
  const restartClient = vscode.commands.registerCommand('holesail-liveports.restartClient', async function (id) {
    const entry = activeClients.get(id)
    if (entry) {
      try {
        await entry.holesail.close()
  ...
  const showConnections = vscode.commands.registerCommand('holesail-liveports.showConnections', async function () {
    const items = []
  ...
  const showOptions = vscode.commands.registerCommand('holesail-liveports.showOptions', function () {
    const options = [
